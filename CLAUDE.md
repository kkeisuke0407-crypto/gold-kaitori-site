...
- **見出し・強調・語り口調の型：`docs/比較記事LPの文章と強調の型.md`** ← 構成より**こっちが本体**。
  セクションの並びは案件で変わってよいが、見出しの立て方・文のリズム・強調の付け方は変えない。
  （1文30字／段落77字／`em-mark`は文まるごと1セクション1〜2個／`em-green`は数字入りの句／`em-red`は否定文・リスク語／素の`strong`が地の強調）
